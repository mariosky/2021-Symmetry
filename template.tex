...
\abstract{ In this work, we propose, through the use of a population-based
    metaheuristic, an optimization method to solve the problem of autonomous
    path tracking using a rear-wheel controller in the framework of fuzzy
    logic. This approach enables the design of controllers using rules that are
    linguistically familiar to human users. Moreover, a new validation
    technique is presented by using three different paths to validate the
    performance of each candidate configuration. We extend on our previous work
    by adding two more membership functions to the previous fuzzy model,
    intending to have a finer grain of adjustment. Experiments show that
    compared to a published control law, the proposed fuzzy controller has a
    better performance when compared with the RMSE but also has limitations
    with respect to undesired yaw oscillation. Nevertheless, the experiments
    also highlight problems with the common practice of evaluating the
    performance of fuzzy controllers with a single problem case and performance
    metric, resulting in controllers  that tend to overtrain.  }
...
Proposed by Lotfi Zadeh \cite{goguen_zadeh_1973}, fuzzy logic introduces the
...
\cite{mamdani1974application,king1977application}. This work establishes the
foundation of fuzzy control \cite{passino1998fuzzy,driankov_introduction_2013},
a technology that has been applied in many research projects
\cite{yang_improved_2003,driankov_fuzzy_2013} and commercial systems.
...
results for rear-weel-based controllers. 
...
membership functions. Also, we propose a new parameterization technique that
...
overtrained controller with low error for a specific track but cannot function
...
\ref{fig:kinematics}) tanget to the path at $s(t)$ given by 
...
previous Section~\ref{sub:FuzzyControllers} need to be tunned to give
...
\begin{specialtable}[H] 
\small
\caption{Ranges defined for each pararamter for the 5MF controller.}\label{tab:factor}
\begin{tabular}{clcl}
\toprule
\textbf{Paramerer}	& \textbf{Range}& \textbf{Parameter} & \textbf{Range}\\
\midrule
a & [0,1]			& f & [0, 1]\\
b & [0.5,2]			& g & [0.5, 2]\\
c & [0,2]			& h & [0, 2]\\
...
\subsubsection{Genetic Algorthm Optimization}\label{sec:GAO}
...
single path; we noticed that this could lead to overtraining. Similar to what
...
was used on prevous work and was taken from the library of
...
computational resources, while the imulations were running, we interrupted
...
\begin{figure}[h]
    \widefigure
     \centering
    \subfigure[Yaw along the refernce path $s(\gamma)$ in meters.]{\label{fig:yawR}\includegraphics[width=90mm]{img/yawR.pdf}}
    \subfigure[Yaw of the control law as it was tracking the refernce path, in seconds.]{\label{fig:yawL}\includegraphics[width=90mm]{img/yawL.pdf}}
    ...
    \subfigure[Yaw of the 5MF controller.]{\label{fig:yaw5}\includegraphics[width=90mm]{img/yaw5.pdf}}
     \caption{Comparation of the yaw (steering) for each of the controllers as it was tracking the reference path ``M'', in seconds.}
        \label{fig:yaw}
\end{figure}
\begin{paracol}{2}
\linenumbers
